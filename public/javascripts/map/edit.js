...
function geoJson2Polys(areajson, bounds){
  ...
  var polygons = _.reduce(coords, function(memo_n, n) {
    var polygonpaths = _.reduce(n, function(memo_o, o) {
      var polygoncords = _.reduce(o, function(memo_p, p) {
...
function onDoneClick() {
    ...
    var url = "/sites/"+siteId+"/protected_areas/"+paId+"/validate";
    $.ajax({
          url: url,
          type: "POST",
          data:{the_geom:polys2geoJson(activePolygons), _method:'put'},
      	  dataType: 'jsonp',
      	  success: function(data) {
...
function saveGeomChanges(has_citation) {
    ...
    var url = "/sites/"+siteId+"/protected_areas/"+paId;
		var geom_data = 'box_to_invalid=' + bbox + '&the_geom=' + polys2geoJson(activePolygons)
    if (has_citation){
				var citation_data = $('#new_dataset *').fieldSerialize();
    }

    $.ajax({
          url: url + ".json",
          type: "POST",
          data: geom_data + "&" + citation_data + "&_method=put",
      		dataType: 'jsonp',
      		success: function(data) {
          if(data.errors.length>0) {
              displayErrorsWindow(data.errors);
